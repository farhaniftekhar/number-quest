...
export interface NumberLineConfig {
  ...
  highlight?: { start: number; end: number; color?: string; label?: string };
}

export class NumberLine {
  constructor(private x: number, private y: number, private width: number) {}

  render(ctx: CanvasRenderingContext2D, config: NumberLineConfig) {
  ...
  private valueToX(value: number, min: number, max: number) {
